...
## Production-ready interview edition

This copy retains the original educational MLOps pipeline and adds production-facing API validation, request/model traceability, readiness checks, deterministic preprocessing, environment-based configuration, least-privilege CI, tests, and non-root container packaging.

For a concise explanation of the architecture, file workflow, design decisions, limitations, and interview answers, read **[INTERVIEW_GUIDE.md](INTERVIEW_GUIDE.md)**.
...
The live classifier also includes a browser UI at `/app`. Once Ray Serve is
running, open `http://127.0.0.1:8000/app` (or append `/app` to your public
tunnel URL) to classify projects without using the Swagger page.
