...
def load_data(dataset_loc: str, num_samples: Optional[int] = None) -> Dataset:
...
def clean_text(text: str, stopwords: List = STOPWORDS) -> str:
    ...
    # Lower and remove URLs before punctuation is split.
    text = text.lower()
...
class CustomPreprocessor:
    ...
    def __init__(self, class_to_index: Optional[Dict] = None):
        self.class_to_index = dict(class_to_index or {})
        self.index_to_class = {v: k for k, v in self.class_to_index.items()}

    def fit(self, ds):
        tags = sorted(ds.unique(column="tag"))
